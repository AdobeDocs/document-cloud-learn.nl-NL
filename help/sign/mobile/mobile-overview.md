---
title: Overzicht van mobiel
description: Verzend documenten ter ondertekening, volg de voortgang van elektronische ondertekening en krijg real-time updates op je mobiele apparaat
feature: Mobile
role: User
level: Intermediate
jira: KT-6854
topic-revisit: Mobile
thumbnail: KT-6854.jpg
exl-id: c52bf1c6-0821-4754-af7f-71f29b8a18c9
source-git-commit: f053f885cce9d41478e581bddeba5fae4196fb91
workflow-type: tm+mt
source-wordcount: '194'
ht-degree: 1%

---

# Overzicht van mobiel

Verzend documenten ter ondertekening, volg de voortgang van elektronische ondertekening en krijg real-time updates op je mobiele apparaat.

## Nieuw

>[!BEGINTABS]

>[!TAB  Mobiel-vriendschappelijke mening ]

Leer hoe te om [&#x200B; mobiel-vriendelijke mening &#x200B;](mobile-friendly.md) te gebruiken om vormen op uw mobiele apparaat te voltooien.

>[!TAB  creeer mobiel-vriendelijke mening ]

Leer hoe te om een [&#x200B; mobiel-vriendelijk &#x200B;](create-mobile-friendly.md) document foutloos, zonder enige ontwikkelaarssteun te produceren.

>[!ENDTABS]

<table style="table-layout:fixed">
<tr>
  <td>
    <a href="sign-mobile.md">
      <img alt="Documenten onderweg ondertekenen" src="../assets/mobile.png" />
    </a>
    <div>
    <a href="sign-mobile.md"><strong> documenten van het Teken op de weg </strong></a>
    </div>
    <em> Leer hoe te om documenten te ondertekenen gebruikend de mobiele app van Acrobat Sign </em>
    <br>
  </td>
  <td>
    <a href="mobile-friendly.md">
      <img alt="Mobielvriendelijke weergave" src="../assets/mobile-friendly.png" />
    </a>
    <div>
    <a href="mobile-friendly.md"><strong> Mobiel-vriendschappelijke mening </strong></a>
    </div>
    <em> Leer hoe te om mobiel-vriendelijke mening te gebruiken om vormen op uw mobiele apparaat te voltooien </em>
    <br>
  </td>  
  <td>
    <a href="create-mobile-friendly.md">
      <img alt="Mobielvriendelijke weergave maken" src="../assets/create-mobile-friendly.png" />
    </a>
    <div>
    <a href="create-mobile-friendly.md"><strong> creeer mobiel-vriendelijke mening </strong></a>
    </div>
    <em> Leer hoe te om een mobiel-vriendelijk document foutloos, zonder enige ontwikkelaarsteun te produceren </em>
    <br>
  </td>
   <td>
    <a href="liquidmode.md">
      <img alt="Vloeiende modus in Acrobat Sign" src="../assets/liquid-mode.png" />
    </a>
    <div>
    <a href="liquidmode.md"><strong> Vloeiende Wijze in Acrobat Sign </strong></a>
    </div>
    <em> Leer hoe de Vloeiende Wijze de mobiele het ondertekenen ervaring verbetert </em>
    <br>
  </td>
</tr>
<tr>
  <td>
    <a href="https://apps.apple.com/us/app/adobe-acrobat-sign/id481082197_blank">
      <img alt="Acrobat Sign mobiele app voor iOS downloaden" src="../assets/mobile-ios.png" />
    </a>
    <div>
    <a href="https://apps.apple.com/us/app/adobe-acrobat-sign/id481082197_blank"><strong> Download Acrobat Sign mobiele app voor iOS </strong></a>
    </div>
    <em> Download Acrobat Sign mobiele app van App Store </em>
    <br>
  </td>
  <td>
    <a href="https://play.google.com/store/apps/details?id=com.adobe.echosign&hl=en&pli=1_blank">
      <img alt="Acrobat Sign mobiele app voor Android downloaden" src="../assets/mobile-android.png" />
    </a>
    <div>
    <a href="https://play.google.com/store/apps/details?id=com.adobe.echosign&hl=en&pli=1_blank"><strong> Download Acrobat Sign mobiele app voor Android </strong></a>
    </div>
    <em> Download Acrobat Sign mobiele app van Google Play </em>
    <br>
  </td>
  <td>
    <img alt="Spacer" src="../assets/Grayspacer.png" />
    <div>
    <br>
  </td>
  <td>
    <img alt="Spacer" src="../assets/Grayspacer.png" />
    <div>
    <br>
  </td>
</tr>
</table>